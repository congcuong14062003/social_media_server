...
class EmailOTP {
  ...
  async create(type) {
    try {
      ...
      // Trả về thời gian hết hạn OTP (15 giây)
  ...
  static async find(user_email, input_code_otp) {
    try {
      const findOTPQuery =
        "SELECT * FROM EmailOTP WHERE user_email = ? AND otp_code = ?;";
  ...
  async isExpiredAndDelete() {
    ...
    if (now > otpExpiration) {
      await this.delete(); // Xóa mã OTP nếu đã hết hạn
      return true;
    }
    return false;
  }
  ...
  async updateStatus(is_verified) {
    try {
      const updateStatusQuery =
        "UPDATE EmailOTP SET is_verified = ? WHERE user_email = ? AND otp_code = ?;";
  ...
  async delete() {
    try {
      const deleteOTPQuery =
        "DELETE FROM EmailOTP WHERE user_email = ? AND otp_code = ?;";
  ...
  async verifyAndDelete(inputOtpCode) {
    try {
      const isExpired = await this.isExpiredAndDelete(); // Kiểm tra OTP hết hạn
      if (isExpired) {
        return "Mã xác thực đã hết hạn!";
      }
      if (this.otp_code === inputOtpCode) {
        await this.updateStatus(true); // Đánh dấu mã OTP đã được sử dụng
        await this.delete(); // Xóa mã OTP sau khi sử dụng
        return true;
      } else {
